...
export class AStarService {
  ...
  getShortestPath(startPosition: Position, endPosition: Position, heuristic: Heuristic = 'distance') {
    ...
    const startNode = this.createPathNode(this.grid.getCell(startPosition), destinationCell, heuristic);
    ...
    while (openList.length) {
      ...
      const destinationNode = adjacentNodes.find(node => node.position.isSame(endPosition));
      if (destinationNode) {
